...
////////////////////////////   CONSTRUCTOR   ///////////////////////////////////
  public GestorBD() {
    ...
    try{
      con = DriverManager.getConnection("jdbc:odbc:basedatosweb", "", "");
...
// Método por el cual añadimos más ventas a un cliente
  ...
  public void PedidoWeb(Vector lineas, String cliente){
    try{
      ...
      while (lineas.size()>0)
      {
        LineaFacturaWeb pedido = (LineaFacturaWeb) lineas.elementAt(0);

        // Sentencia SQL
        String sentencia = "INSERT INTO pedidosweb VALUES ('" + cliente +
            "','" + pedido.getArticulo() + "','" + pedido.getUnidades() +
            "','" + fecha1 + "')";

        // La ejecutamos
      stmt.executeUpdate(sentencia);

      lineas.removeElementAt(0);
      }
      commit();
    }

    catch (Exception e){
      System.out.println("Error en tabla de Web");
      System.out.println(e.getMessage());
    }
  ...
  public Vector ExtracosEntreFechas(String desde, String hasta, String cod_cli){
    // Hemos modificado la clase FechaFacturaWeb para poderla usar aquí;
    // Utilizaremos el constructor
    // public LineaFacturaWeb(String fecha, String articulo, String unidades)

    ResultSet rs = null;              // Para la base de datos
    LineaFacturaWeb linea;            // Para guardar la linea temporalmente
    LineaFacturaWeb l;
    Vector mislineas = new Vector();  // Vector donde almacenaremos todas las lineas


    // Primero lanzaremos la sentencia SQL
    try{

      String sentencia = "SELECT * FROM historico WHERE '"+cod_cli+"'=codigocp AND fecha BETWEEN '"+desde+"' AND '"+hasta+"'";
      ...
      /*System.out.println("Error SQL al buscar Extractos");
      System.out.println(e.getMessage());*/
    }

    // Y después recuperaremos la linea
    try{
      while (rs.next() == true){                   // Si existe esa linea
        // Creamos una linea temporal
        linea = new LineaFacturaWeb (rs.getString("fecha"),
                                 rs.getString("codarticulo"),rs.getString("unidades"));
        // Y se lo pasamos al Vector
        mislineas.add(linea);
      }//while
      ...
      /*System.out.println("Error en la búsqueda de Lineas");
      System.out.println(e.getMessage());*/
    }
    return mislineas;
  }//ExtractosEntreFechas
